...
# НОВИЙ АРКУШ: для штрафів та утримань
WITHHOLDING_WORKSHEET_TITLE = "Утримання"
...
# НОВІ ЗАГОЛОВКИ: для штрафів та утримань
WITHHOLDING_HEADERS = ["Дата", "ПІБ Водія", "Сума штрафу, грн", "Примітка", "Зафіксував"]
...
# НОВА ФУНКЦІЯ: Збереження штрафу в аркуш "Утримання"
async def append_penalty_to_gsheet(penalty_data: dict):
    """Зберігає штраф в відповідний аркуш."""
    worksheet = await get_gsheet_worksheet(WITHHOLDING_WORKSHEET_TITLE, WITHHOLDING_HEADERS)
    if worksheet:
        row = [
            datetime.now().strftime('%d.%m.%Y %H:%M'),
            penalty_data.get("driver_full_name"),
            penalty_data.get("amount"),
            penalty_data.get("note", ""),
            penalty_data.get("created_by")
        ]
        await asyncio.to_thread(worksheet.append_row, [str(item) for item in row], value_input_option='USER_ENTERED')
        logging.info(f"Збережено штраф для {penalty_data.get('driver_full_name')} на суму {penalty_data.get('amount')} грн")
...
class PenaltyManagement(StatesGroup):
    selecting_driver = State()
...
NOTE_EDIT_KEYBOARD = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📝 Немає примітки")], [KeyboardButton(text="❌ Скасувати")]], resize_keyboard=True)
...
def get_accountant_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💸 Зафіксувати штраф", callback_data="create_penalty")],
        [InlineKeyboardButton(text="↩️ Назад до головного меню", callback_data="back_to_main_menu")]
    ])
...
def get_trip_edit_keyboard(has_note: bool = False) -> InlineKeyboardMarkup:
    note_button_text = "✏️ Примітка" if has_note else "➕ Додати примітку"
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=note_button_text, callback_data="edit_trip_note")],
...
# Overridden version with pagination support
async def get_tour_selection_keyboard(page: int = 1, per_page: int = 10) -> InlineKeyboardMarkup:
    tours = await get_tours_from_gsheet()
    total = len(tours)
    if page < 1:
        page = 1
    ...
    page_items = tours[start:end]

    buttons = []
    for tour in page_items:
        # Expecting keys 'Номер туру' and 'Відстань км'. Fallback to first keys if encoding differs.
        tour_num = tour.get('Номер туру', tour.get(next(iter(tour.keys()), '')))
        tour_dist = tour.get('Відстань км', '')
        tour_text = f"№-{tour_num} ({tour_dist} км)"
        buttons.append([InlineKeyboardButton(text=tour_text, callback_data=f"select_tour_{tour_num}")])

    # Navigation row (Prev | Page X/Y | Next)
    total_pages = max(1, (total + per_page - 1) // per_page)
    nav_row = []
    if page > 1:
        nav_row.append(InlineKeyboardButton(text="« Назад", callback_data=f"tours_page_{page-1}"))
    nav_row.append(InlineKeyboardButton(text=f"Сторінка {page}/{total_pages}", callback_data="noop"))
    if end < total:
        nav_row.append(InlineKeyboardButton(text="Вперед »", callback_data=f"tours_page_{page+1}"))
    if nav_row:
        buttons.append(nav_row)

    # Extra actions
    buttons.append([InlineKeyboardButton(text="Ввести вручну номер туру", callback_data="manual_tour_entry")])
    buttons.append([InlineKeyboardButton(text="День відпочинку", callback_data="tour_day_off")])
    buttons.append([CANCEL_BUTTON_INLINE])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
async def show_trip_data_for_confirmation(message_or_callback: types.Message | types.CallbackQuery, state: FSMContext):
    ...
    keyboard = get_trip_edit_keyboard(has_note=bool(note))
...
@dp.callback_query(F.data.startswith("tours_page_"), Reporting.waiting_for_tour_number)
async def paginate_tours(callback: types.CallbackQuery, state: FSMContext):
    await safe_callback_answer(callback)
    try:
        page = int(callback.data.split("tours_page_")[1])
    except Exception:
        page = 1
    keyboard = await get_tour_selection_keyboard(page=page)
    text = "**Оберіть тур/маршрут** або введіть вручну:"
    await safe_edit_message(callback, text, parse_mode="Markdown", reply_markup=keyboard)

@dp.callback_query(F.data == "noop")
async def noop_handler(callback: types.CallbackQuery):
    # Do nothing, just acknowledge to remove 'loading' state
    await safe_callback_answer(callback)
...
async def process_actual_refill_input(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("💡 Введіть примітку до рейсу або натисніть кнопку '📝 Немає примітки':", reply_markup=NOTE_EDIT_KEYBOARD)
...
async def process_note_input(message: types.Message, state: FSMContext):
    ...
    if note.lower() in ['немає', 'нема', 'без примітки', '-', ''] or note == '📝 Немає примітки':
        note = ""
...
async def edit_trip_note(callback: types.CallbackQuery, state: FSMContext):
    await safe_callback_answer(callback)
    await safe_edit_message(callback, "Введіть нову примітку або оберіть варіант:")
    await callback.message.answer("💡 Введіть текст примітки або натисніть кнопку '📝 Немає примітки'", reply_markup=NOTE_EDIT_KEYBOARD)
    await state.set_state(Reporting.waiting_for_note)
...
@dp.callback_query(F.data == "create_penalty")
async def start_penalty_creation(callback: types.CallbackQuery, state: FSMContext):
    await safe_callback_answer(callback)
    user_data = await get_user_from_gsheet(str(callback.from_user.id))
    if user_data.get("role") not in ["ТО", "бухгалтер", "адмін"]:
        await safe_edit_message(callback, "❌ Немає доступу.")
        return

    # Отримуємо список всіх водіїв
    worksheet = await get_gsheet_worksheet(USERS_WORKSHEET_TITLE, USERS_HEADERS)
    if not worksheet:
        await safe_edit_message(callback, "❌ Помилка отримання даних.")
        return

    records = await asyncio.to_thread(worksheet.get_all_records)
    drivers = [r for r in records if r.get('role', '').strip().lower() == 'водій']

    if not drivers:
        await safe_edit_message(callback, "❌ Водіїв не знайдено.")
        return

    # Створюємо клавіатуру з водіями
    buttons = []
    for driver in drivers:
        full_name = f"{driver.get('first_name', '')} {driver.get('last_name', '')}".strip()
        if full_name:
            buttons.append([InlineKeyboardButton(text=f"👤 {full_name}", callback_data=f"penalty_driver_{driver.get('user_id')}")])

    buttons.append([InlineKeyboardButton(text="❌ Скасувати", callback_data="back_to_penalty_panel")])
    keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)

    await safe_edit_message(callback, "💸 **Зафіксувати штраф**\n\nОберіть водія:", reply_markup=keyboard, parse_mode="Markdown")
    await state.set_state(PenaltyManagement.selecting_driver)

@dp.callback_query(F.data.startswith("penalty_driver_"), PenaltyManagement.selecting_driver)
async def select_penalty_driver(callback: types.CallbackQuery, state: FSMContext):
    await safe_callback_answer(callback)
    user_id = callback.data.split("_")[-1]

    # Отримуємо дані водія
    driver_data = await get_user_from_gsheet(user_id)
    if not driver_data:
        await safe_edit_message(callback, "❌ Водія не знайдено.")
        return

    driver_full_name = f"{driver_data.get('first_name', '')} {driver_data.get('last_name', '')}".strip()
    ...
    await safe_edit_message(callback, f"💸 **Штраф для: {driver_full_name}**\n\nВведіть суму штрафу (грн):")
    await callback.message.answer("💡 Введіть суму числом (наприклад: 500)", reply_markup=CANCEL_KEYBOARD_REPLY)
    await state.set_state(PenaltyManagement.entering_amount)

@dp.message(PenaltyManagement.entering_amount)
async def process_penalty_amount(message: types.Message, state: FSMContext):
    try:
        amount = float(message.text.replace(',', '.'))
        if amount <= 0:
            await message.answer("❗️ Сума має бути більше 0. Введіть ще раз:", reply_markup=CANCEL_KEYBOARD_REPLY)
            return

        await state.update_data(amount=amount)
        data = await state.get_data()

        await message.answer(f"💸 **Штраф для: {data.get('driver_full_name')}**\n💰 **Сума: {amount} грн**\n\nВведіть примітку до штрафу:")
        await message.answer("💡 Введіть причину штрафу або натисніть кнопку '📝 Немає примітки'", reply_markup=NOTE_EDIT_KEYBOARD)
        await state.set_state(PenaltyManagement.entering_note)
    except ValueError:
        await message.answer("❗️ Невірний формат. Введіть суму числом:", reply_markup=CANCEL_KEYBOARD_REPLY)

@dp.message(PenaltyManagement.entering_note)
async def process_penalty_note(message: types.Message, state: FSMContext):
    note = message.text.strip()
    if note == '📝 Немає примітки' or note.lower() in ['немає', 'нема', 'без примітки', '-', '']:
        note = ""
    ...
    text = (f"💸 **Підтвердіть штраф:**\n\n"
            f"👤 **Водій**: `{data.get('driver_full_name')}`\n"
            f"💰 **Сума**: `{data.get('amount')} грн`\n"
            f"{note_text}\n"
            f"Все вірно?")

    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Зафіксувати штраф", callback_data="confirm_penalty")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="back_to_penalty_panel")]
    ])

    await message.answer(text, reply_markup=keyboard, parse_mode="Markdown")
    await state.set_state(PenaltyManagement.confirming)

@dp.callback_query(F.data == "confirm_penalty", PenaltyManagement.confirming)
async def confirm_penalty(callback: types.CallbackQuery, state: FSMContext):
    await safe_callback_answer(callback)
    await safe_edit_message(callback, "⏳ Фіксую штраф...", reply_markup=None)

    data = await state.get_data()
    user_data = await get_user_from_gsheet(str(callback.from_user.id))
    creator_name = f"{user_data.get('first_name', '')} {user_data.get('last_name', '')}".strip()

    penalty_data = {
        "driver_full_name": data.get("driver_full_name"),
    ...
    try:
        await append_penalty_to_gsheet(penalty_data)
        await safe_edit_message(callback, f"✅ **Штраф зафіксовано!**\n\n"
                                         f"👤 Водій: {data.get('driver_full_name')}\n"
                                         f"💰 Сума: {data.get('amount')} грн\n"
                                         f"📝 Примітка: {data.get('note') or 'немає'}")
        await state.clear()
    except Exception as e:
        logging.error(f"Помилка збереження штрафу: {e}")
        await safe_edit_message(callback, "❌ Помилка збереження штрафу.")

@dp.callback_query(F.data == "back_to_penalty_panel")
async def back_to_penalty_panel(callback: types.CallbackQuery, state: FSMContext):
    await safe_callback_answer(callback)
    await state.clear()
